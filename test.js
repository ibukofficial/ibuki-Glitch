...
const { Client, GatewayIntentBits, Partials, EmbedBuilder, ApplicationCommandType, ApplicationCommandOptionType } = require('discord.js'), //これ使って一部の機能が使える。
	client = new Client({
		...
		intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildVoiceStates, GatewayIntentBits.GuildMessages, GatewayIntentBits.GuildMessageReactions, GatewayIntentBits.DirectMessages, GatewayIntentBits.MessageContent]
	}), //インテントとは、メッセージにが送信されたことを取得したり、最近だとメッセージ内容を取得するかどうがを決める
	data = require("./data.json"); //json取り込み
var args, cmdname, channellog, br = "\n", bro = "```", scommand = [{ name: "help", description: "ヘルプを表示します。" }], helpdata = "```", texts = data.texts;
for (var i = 0; i != data.data.length; i++) {
	scommand.push({ name: data.data[i].command, description: data.data[i].name });
};
...
client.on('interactionCreate', interaction => {
	try {
		...
		if (cmdname == "help") {
			...
			for (var i = 0; i != data.data.length; i++) { //dataの数だけ
				if (cmdname == data.data[i].command) { //コマンドがあってるか繰り返す(ループして一つ一つ当たるかifで判断)
